...
if any(mount.mountPoint == "/mnt/xxxxxxxxxxxxx" for mount in mounts):
    ...
    dbutils.fs.mount(
        source="s3a://xxxxxxxxxxx",
        mount_point="/mnt/xxxxxxxxxxxxxx",
        extra_configs={
...
# Carregar os dados do CSV no Spark DataFrame
file_path = "/mnt/xxxxxxxxxxxx/Hospital General Information.csv"
hospital_df = spark.read.csv(file_path, header=True, inferSchema=True)
...
# Carregar os dados do CSV no Spark DataFrame
file_path = "/mnt/xxxxxxxxxxxx/Hospital General Information.csv"
df_final = spark.read.csv(file_path, header=True, inferSchema=True)
...
os.environ["AWS_SECRET_ACCESS_KEY"] = "xxxxxxxxxxxxxxxxxxxxxx"

# Detalhes do cluster Redshift
redshift_endpoint = "xxxxxxxxxxxxx.ccwmbo9ozeli.us-east-1.redshift.amazonaws.com"
redshift_db = "dev"  # Nome do banco 
redshift_user = "xxxxxxxx"
redshift_password = "xxxxxxxxxxxx"
redshift_port = 5439  # Porta padrão para Redshift

# Criação da string de conexão usando sqlalchemy-redshift
connection_string = f"redshift+psycopg2://{redshift_user}:{redshift_password}@{redshift_endpoint}:{redshift_port}/{redshift_db}"

# Criação do engine de conexão
engine = create_engine(connection_string)

# Carregar os dados para o Redshift
try:
    pandas_df.to_sql('df_final', engine, index=False, if_exists='replace', method='multi')
